...
def test_page_score_can_rerank_results():
    cfg = SearchConfig(default_profile="general", profiles={"general": SearchContextConfig()})
    ...
    response = {
        "results": [
            {"url": "https://a.example/1", "title": "t1", "snippet": "hello s1", "engine": "x"},
            {"url": "https://a.example/2", "title": "t2", "snippet": "hello s2", "engine": "x"},
            {"url": "https://a.example/3", "title": "t3", "snippet": "hello s3", "engine": "x"},
        ]
    }

    ctx = pipeline.build_context(response, "hello", "high", max_results=10)
    # Snippet ranking would be 1 > 3 > 2, but page makes 3 outrank 1.
    assert [r.url for r in ctx.results] == ["https://a.example/3", "https://a.example/1"]
