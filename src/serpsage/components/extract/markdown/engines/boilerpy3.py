...
try:
    ...
    _BOILERPY3_AVAILABLE = True
except Exception:  # noqa: BLE001
    extractors = None
    _BOILERPY3_AVAILABLE = False


def boilerpy3_available() -> bool:
    return bool(_BOILERPY3_AVAILABLE and extractors is not None)
...
def run_boilerpy3(
    *,
    ...
    if not boilerpy3_available() or extractors is None:
        return None

    try:
        extractor = extractors.ArticleExtractor()
        text = extractor.get_content(html_doc) or ""
    except Exception as exc:  # noqa: BLE001
        return CandidateDoc(
            markdown="",
            plain_text="",
            extractor_used="boilerpy3",
            quality_score=0.0,
            warnings=[f"boilerpy3_failed:{type(exc).__name__}"],
            stats={"engine": "boilerpy3"},
            primary_chars=0,
            secondary_chars=0,
        )
    ...
    if len(plain) < int(profile.min_plain_chars):
        warnings.append("boilerpy3 low text output")
