...
class _ScoringEngineBase:
    """Shared scoring utilities for sync/async engines."""
    ...
    def _get_heuristic_scores_impl(
        self,
        texts: list[str],
        *,
        query: str,
        ...
        """Compute raw heuristic scores (sync implementation)."""
        hcfg = self.config.scoring.heuristic
        q_tokens = (
    ...
    def _get_bm25_scores_impl(
        self, docs: list[str], *, query: str, **_: Any
    ) -> list[float]:
        """Compute BM25 scores (sync implementation)."""
        if not docs:
            return []
    ...
    def normalize_scores(self, scores: list[float]) -> list[float]:
        ...
        if spread < float(self.config.scoring.normalization.flat_spread_eps):
            return [0.0 for _ in cleaned]

        method = (self.config.scoring.normalization.method or "robust_sigmoid").lower()
        if method == "rank" or n < int(
            self.config.scoring.normalization.min_items_for_sigmoid
        ):
        ...
        z_clip = max(0.0, float(self.config.scoring.normalization.z_clip))
        temp = (
            float(self.config.scoring.normalization.temperature)
            if float(self.config.scoring.normalization.temperature) > 0
            else 1.0
        )
    ...
    def normalize_provider_weights(self) -> dict[str, float]:
        ...
        raw = {
            k: float(v)
            for k, v in (self.config.scoring.providers or {}).items()
            if float(v) > 0
        }
...
class ScoringEngine(_ScoringEngineBase):
    """Sync scoring engine."""
    ...
    def get_heuristic_scores(
        ...
        *,
        ...
        return self._get_heuristic_scores_impl(
            texts,
            query=query,
            query_tokens=query_tokens,
            intent_tokens=intent_tokens,
    ...
    def get_bm25_scores(self, docs: list[str], *, query: str, **_: Any) -> list[float]:
        ...
        """
        return self._get_bm25_scores_impl(docs, query=query)


class AsyncScoringEngine(_ScoringEngineBase):
    """Async scoring engine.

    The CPU-heavy scoring steps are executed in a worker thread to avoid blocking
    the event loop.
    """
    ...
    async def score(
        ...
        *,
        ...
        heuristic_raw = await self.get_heuristic_scores(
            texts,
            query=query,
            query_tokens=query_tokens,
            intent_tokens=intent_tokens,
        ...
        blended = [float(s) * heur_w for s in heuristic_raw]
        if bm25_w > 0:
            bm25_raw = await self.get_bm25_scores(texts, query=query)
            scaled = self.rank_scales(bm25_raw)
            max_heur = max(heuristic_raw) if heuristic_raw else 0.0
            anchor = float(max_heur) if float(max_heur) > 0 else 1.0
            for i in range(len(blended)):
    ...
    async def get_heuristic_scores(
        self,
        texts: list[str],
        *,
        query: str,
        ...
        return await to_thread.run_sync(
            lambda: self._get_heuristic_scores_impl(
                texts,
                query=query,
                query_tokens=query_tokens,
                intent_tokens=intent_tokens,
    ...
    async def get_bm25_scores(
        self, docs: list[str], *, query: str, **_: Any
    ) -> list[float]:
        return await to_thread.run_sync(
            lambda: self._get_bm25_scores_impl(docs, query=query)
        )
